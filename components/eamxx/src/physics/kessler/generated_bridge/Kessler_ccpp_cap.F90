module Kessler_ccpp_cap
  ...
  public :: ccpp_register
  public :: ccpp_init
  public :: ccpp_final
  public :: ccpp_physics_run
  public :: ccpp_physics_timestep_init
  public :: ccpp_physics_timestep_final
  public :: ccpp_physics_init
  public :: ccpp_physics_final
...
CONTAINS
  ...
  subroutine ccpp_register(suite_name, errmsg, errflg) BIND(C, name='ccpp_register') 
  ...
  subroutine ccpp_init(suite_name, errmsg, errflg) BIND(C, name='ccpp_init') 
    character(kind=c_char, len=1), intent(in) :: suite_name(*)
    character(kind=c_char, len=1), intent(out) :: errmsg(*)
    integer(c_int), intent(out) :: errflg
    integer :: ccpp_c2f_i
  ...
  subroutine ccpp_final(suite_name, errmsg, errflg) BIND(C, name='ccpp_final') 
    character(kind=c_char, len=1), intent(in) :: suite_name(*)
    character(kind=c_char, len=1), intent(out) :: errmsg(*)
    integer(c_int), intent(out) :: errflg
    integer :: ccpp_c2f_i
    ...
    errmsg(len_trim(errmsg_f)+1) = c_null_char
  end subroutine ccpp_final 
  
  subroutine ccpp_physics_run(suite_name, suite_part, col_start, col_end, errmsg, errflg) BIND(C, &
    name='ccpp_physics_run') 
    character(kind=c_char, len=1), intent(in) :: suite_name(*)
    ...
    if (trim(suite_name_f) .eq. 'kessler_suite') then
      ...
      if (trim(suite_part_f) .eq. 'physics') then
        ...
        !$acc      exner(col_start:col_end, 1:nz)) copyout(precl(col_start:col_end), &
        !$acc      relhum(col_start:col_end, 1:nz))
#endif
        call kessler_suite_physics(ncol, nz, dt, lyr_surf, lyr_toa,                               &
          ...
          1:nz), qr(col_start:col_end, 1:nz), precl(col_start:col_end), relhum(col_start:col_end, &
          1:nz), temp_prev(col_start:col_end, 1:nz), temp_tend(col_start:col_end, 1:nz),          &
          scheme_name, errmsg_f, errflg)
#ifdef USE_GPU
        !$acc end data
#endif
#ifdef USE_GPU
  ...
  subroutine ccpp_physics_timestep_init(suite_name, suite_part, col_start, col_end, errmsg,       &
    errflg) BIND(C, name='ccpp_physics_timestep_init') 
    character(kind=c_char, len=1), intent(in) :: suite_name(*)
    character(kind=c_char, len=1), intent(in) :: suite_part(*)
    integer(c_int), value, intent(in) :: col_start
    integer(c_int), value, intent(in) :: col_end
...
#endif
        call kessler_suite_timestep_init_physics(ncol, nz, temp(col_start:col_end, 1:nz),         &
          temp_prev(col_start:col_end, 1:nz), temp_tend(col_start:col_end, 1:nz), errmsg_f, errflg)
      else
        write(errmsg_f, '(3a)') "No suite part named ", trim(suite_part_f),                       &
          " found in suite kessler_suite"
        errflg = 1        
  ...
  subroutine ccpp_physics_timestep_final(suite_name, suite_part, col_start, col_end, errmsg,      &
    errflg) BIND(C, name='ccpp_physics_timestep_final') 
    character(kind=c_char, len=1), intent(in) :: suite_name(*)
    character(kind=c_char, len=1), intent(in) :: suite_part(*)
    integer(c_int), value, intent(in) :: col_start
    integer(c_int), value, intent(in) :: col_end
    ...
    if (trim(suite_name_f) .eq. 'kessler_suite') then
      ...
      if (trim(suite_part_f) .eq. 'physics') then
#ifdef USE_GPU
        !$acc data copyin(phis(col_start:col_end)) copyout(st_energy(col_start:col_end, 1:nz))
#endif
        call kessler_suite_timestep_final_physics(nz, ncol, cpair(col_start:col_end, 1:nz),       &
          temp(col_start:col_end, 1:nz), z_mid(col_start:col_end, 1:nz), phis(col_start:col_end), &
          st_energy(col_start:col_end, 1:nz), errmsg_f, errflg)
#ifdef USE_GPU
        !$acc end data
#endif
#ifdef USE_GPU
    ...
    errmsg(len_trim(errmsg_f)+1) = c_null_char
  end subroutine ccpp_physics_timestep_final 
  
  subroutine ccpp_physics_init(suite_name, suite_part, col_start, col_end, errmsg,                &
    errflg) BIND(C, name='ccpp_physics_init') 
    character(kind=c_char, len=1), intent(in) :: suite_name(*)
    character(kind=c_char, len=1), intent(in) :: suite_part(*)
    integer(c_int), value, intent(in) :: col_start
    integer(c_int), value, intent(in) :: col_end
    ...
    if (trim(suite_name_f) .eq. 'kessler_suite') then
      if (trim(suite_part_f) .eq. 'physics') then
        call kessler_suite_init_physics(lv, pref, rhoqr, gravit, errmsg_f, errflg)
      else
        write(errmsg_f, '(3a)') "No suite part named ", trim(suite_part_f),                       &
          " found in suite kessler_suite"
        errflg = 1        
  ...
  subroutine ccpp_physics_final(suite_name, suite_part, col_start, col_end, errmsg,               &
    errflg) BIND(C, name='ccpp_physics_final') 
    character(kind=c_char, len=1), intent(in) :: suite_name(*)
    character(kind=c_char, len=1), intent(in) :: suite_part(*)
    integer(c_int), value, intent(in) :: col_start
    integer(c_int), value, intent(in) :: col_end
    ...
    errmsg(len_trim(errmsg_f)+1) = c_null_char
  end subroutine ccpp_physics_final 

...
from kessler_jax.kessler_update_run import kessler_update_run_core
...
@functools.partial(jax.jit, static_argnames=('nz', 'ncol', 'errflg',))
def kessler_update_run_bridge_device(nz, ncol, dt, theta, exner, temp_prev, ttend_t, errflg):
    """
    DEVICE-RESIDENT bridge entry for kessler_update_run (contract 2, no transfers).

    Same arguments as kessler_update_run_bridge minus CHARACTER params, in the
    ...
    are bit-identical to kessler_update_run_bridge's (same jitted code — the
    ...
    exner = jnp.transpose(exner)
    temp_prev = jnp.transpose(temp_prev)
    ttend_t = jnp.transpose(ttend_t)
    ttend_t, errflg = kessler_update_run_core(nz, ncol, dt, theta, exner, temp_prev, ttend_t, errflg)
    # In-jit output reversal: back to Fortran (ncol, nz) order
    ttend_t = jnp.transpose(ttend_t)
    return ttend_t, errflg

_kessler_update_run_device = kessler_update_run_bridge_device  # alias (private name kept for compatibility)

def kessler_update_run_bridge(nz, ncol, dt, theta, exner, temp_prev, ttend_t, errmsg, errflg):
    """
    Production HOST-FACING bridge for kessler_update_run (contract 1).

    Device-resident callers use kessler_update_run_bridge_device instead
    (contract 2: device arrays in/out, no transfers); this function is
    exactly to_device -> kessler_update_run_bridge_device -> batched device_get.
    ...
    kessler_update_run_core, reverse back in-jit, pure D2H. Inputs and outputs
    ...
    exner_dev = to_device(exner)
    temp_prev_dev = to_device(temp_prev)
    ttend_t_dev = to_device(ttend_t)

    # Compute — in-jit layout conversion + jitted core
    ttend_t_out, errflg_out = kessler_update_run_bridge_device(nz=nz, ncol=ncol, dt=dt, theta=theta_dev, exner=exner_dev, temp_prev=temp_prev_dev, ttend_t=ttend_t_dev, errflg=errflg)

    # Convert outputs — ONE batched D2H (single sync); layout already
    # Fortran (ncol, nz) C-order
    ttend_t_host, errflg_host = jax.device_get((ttend_t_out, errflg_out))
    ttend_t_fortran = np.asarray(ttend_t_host, dtype=np.float64)
    errflg_fortran = np.asarray(errflg_host).item()

    return ttend_t_fortran, errmsg, errflg_fortran

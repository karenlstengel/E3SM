...
void KesslerMicrophysics::set_grids(const std::shared_ptr<const GridsManager> grids_manager)
{
  ...
  // TODO - set up all of the fields from the manager needed by Kessler here
}
...
void KesslerMicrophysics::initialize_impl (const RunType /* run_type */)
{
  ...
  kessler::kessler_eamxx_bridge_init(m_num_cols, m_num_levs);
}
...
//   buffer_size+= KMF:params::num_2d_midlv_c_views * sizeof(Spack) * m_num_cols * nlevm_packs;
//   buffer_size+= KMF:params::num_2d_midlv_f_views * sizeof(Real)  * m_num_cols * m_num_levs;
...
//   constexpr auto num_2d_midlv_c_views = KMF:params::num_2d_midlv_c_views;
//   constexpr auto num_2d_midlv_f_views = KMF:params::num_2d_midlv_f_views;
  
//   //----------------------------------------------------------------------------
//   Real* r_mem = reinterpret_cast<Real*>(buffer_manager.get_memory());
//   //----------------------------------------------------------------------------
//   // 2D "f_" views on mid-point levels
//   KMF:view_2dl<Real>* midlv_f_ptrs[num_2d_midlv_f_views]  = { &params.f_p_mid, &params.f_T_mid};
...
//   KMF:view_2d<Spack>* midlv_c_ptrs[num_2d_midlv_c_views]  = { &params.p_mid, &params.T_mid};
